      ...
      KakaoLogins.login()
        .then(result => {
          ...
          .then(result => {
            ...
            return fetch(`http://3mstudy.kr/app/api/?cmd=set_check_id_in_profile&mb_id=${result.id}`)
            ...
            .then((responseJson) => {
              if (responseJson) {
                ...
                fetch(`http://3mstudy.kr/app/api/?cmd=register&mode=kakao&id=${result.id}&name=${result.nickname}&email=${global.id3}&profileImgUrl=${result.profile_image_url}&img_path=${result.profile_image_url}`)
                .then((response) => response.json())
                .then((responseJson) => {
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
          <ImageBackground style={{ flex: 1 }} source={require("./images/Login_Background.png")} resizeMode="cover">
